...
function textNodesToWords(el: HTMLElement): string[][] {
...
export function splitWords(el: HTMLElement): void {
  ...
  lines.forEach((words, lineIndex) => {
    words.forEach((word, i) => {
      ...
      w.style.setProperty('--word-index', String(wordIndex))
      w.style.setProperty('--line-index', String(lineIndex))
  ...
  el.innerHTML = ''
  el.appendChild(frag)
  el.style.setProperty('--word-total', String(wordIndex))
...
export function splitChars(el: HTMLElement): void {
  ...
  lines.forEach((words, lineIndex) => {
    words.forEach((word, i) => {
      ...
      Array.from(word).forEach((ch) => {
        ...
        c.style.setProperty('--char-index', String(charIndex))
        c.textContent = ch
        w.appendChild(c)
        charIndex++
      })
  ...
  el.innerHTML = ''
  el.appendChild(frag)
  el.style.setProperty('--char-total', String(charIndex))
  el.classList.add('chars', 'splitting')
}

export function runSplitting(root: Document | HTMLElement = document): void {
  root.querySelectorAll<HTMLElement>('.split-words:not(.splitting)').forEach(splitWords)
  root.querySelectorAll<HTMLElement>('.split-chars:not(.splitting)').forEach(splitChars)
}
