...
export function tonoDelColor(color: string): "dark" | "" | null {
...
export function tonoEn(x: number, y: number): "dark" | "" | null {
...
export function initSectionWatcher(): Cleanup {
  ...
  const medir = (el: HTMLElement, respaldo: string) => {
    ...
    const marca =
      el.querySelector<HTMLElement>(".logo .logo-d, #bt-menu") ?? el;
  ...
  const aplicar = () => {
    ...
    if (tLogo !== null && logo.dataset.tono !== tLogo)
      logo.dataset.tono = tLogo;
    if (tBoton !== null && boton.dataset.tono !== tBoton)
      boton.dataset.tono = tBoton;
  };
  raf = requestAnimationFrame(aplicar);
